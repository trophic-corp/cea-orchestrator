...
**Owner decision recorded 2026-09-10** (binding on the docs set):

7. **The Phase 1 build pause is lifted.** Phase 1 runs on the simulator (FR-SIM-01,
   virtual rack + virtual room controller) while OQ-3 stays open in parallel. Phase 1
   contains no actuation and no real room-controller firmware, so no Phase 1 deliverable
   depends on the platform choice; OQ-3 remains the gate for *real* room-controller
   firmware (R1/R2 hardware validation), not for the software spine.
...
| AR-2 | **Room controller platform undefined** (OQ-3) — R1–R2 firmware design could be built on the wrong compute assumption. | M/H | **Being actively closed:** [decision brief](../pipeline/oq3-room-controller-platform/00-decision-brief.md) ready 2026-09-08. The Phase 1 build was paused behind it 2026-09-08 and **un-paused 2026-09-10** to run on the simulator — the room controller is a virtual device in Phase 1, so nothing built is platform-specific. Note the hardware side (`trophic-hardware` CT-B product doc) is waiting on this decision from the software side while the brief §6 asks the hardware program for enclosure/procurement input — **the owner has to break that circular wait** by answering brief §6 questions 1–2. Closes with ADR-0007. |
...
| 9 | ~~Phase 1 (Release 1) kickoff~~ — **STARTED 2026-09-08** on owner go-ahead. Decisions recorded (ADRs accepted, OQ-1 closed, **OQ-16 → Go, ADR-0006**). Build paused 2026-09-08 pending OQ-3; **pause lifted 2026-09-10 — Phase 1 runs on the simulator** (`/ship` kicked off, artifacts under `docs/pipeline/phase-1-foundation/`). OQ-3 (action #5) proceeds in parallel. | Pipeline | **Running** |
